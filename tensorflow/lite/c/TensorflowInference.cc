...
public: 
    ...
    { 
        DebugMessage(m_logger) << "\nStart TensorFlow 2.8 with coreML support init function on " << modelFileName; 
        ...
        //coreMlDelegateOptions.coreml_version = 2; 
  
        m_coreMl_delegate = TfLiteCoreMlDelegateCreate(&coreMlDelegateOptions); 
  
        if (m_interpreter->ModifyGraphWithDelegate(m_coreMl_delegate) == kTfLiteOk) { 
            DebugMessage(m_logger) << "\nTensorflow init coreMl successfully"; 
        } else { 
            ErrorMessage(m_logger) << "\nTensorflow cant init coreMl"; 
        } 
